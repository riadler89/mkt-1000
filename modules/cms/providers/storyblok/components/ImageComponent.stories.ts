...
/**
 * The Image component displays responsive images with support for different aspect ratios
 * on desktop and mobile devices. It uses Storyblok assets with proper fallback handling.
 */
interface ImageStoryArgs {
  desktopImageUrl: string
  mobileImageUrl: string
  altText: string
  aspectRatioDesktop: ImageComponent['aspectRatioDesktop']
  aspectRatioMobile: ImageComponent['aspectRatioMobile']
}
...
const createContentElement = (
  ...
  mobileImageUrl: string,
  altText: string,
  aspectRatioDesktop: ImageComponent['aspectRatioDesktop'] = 'original',
  aspectRatioMobile: ImageComponent['aspectRatioMobile'] = 'original',
): ImageComponent => ({
  imageDesktop: desktopImageUrl
    ? ({
        filename: desktopImageUrl,
        meta_data: { size: '1280x1455' },
      } as unknown as StoryblokAsset)
    : undefined,
  imageMobile: mobileImageUrl
    ? ({
        filename: mobileImageUrl,
        meta_data: { size: '1280x1455' },
      } as unknown as StoryblokAsset)
    : undefined,
  altText,
...
const meta = {
  title: 'CMS/Storyblok/Image',
  component: Image,
  argTypes: {
    desktopImageUrl: {
      control: 'text',
      description: 'Desktop image URL',
      name: 'contentElement.imageDesktop.filename',
    },
    mobileImageUrl: {
      control: 'text',
      description: 'Mobile image URL (optional)',
      name: 'contentElement.imageMobile.filename',
    },
    altText: {
      control: 'text',
      description: 'Alternative text for accessibility',
      name: 'contentElement.altText',
    },
    aspectRatioDesktop: {
      control: 'select',
      options: ['', 'original', '1:1', '16:9', '4:3'],
      description: 'Aspect ratio for desktop display',
      name: 'contentElement.aspectRatioDesktop',
    },
    aspectRatioMobile: {
      control: 'select',
      options: ['', 'original', '1:1', '16:9', '4:3'],
      description: 'Aspect ratio for mobile display',
      name: 'contentElement.aspectRatioMobile',
    },
  },
  ...
  parameters: {
    docs: {
      ...
      description: {
        component:
          'A responsive image component that displays Storyblok assets with configurable aspect ratios for desktop and mobile viewports.',
      },
    },
  },
}
